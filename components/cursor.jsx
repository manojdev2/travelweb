...
function CursorFollow() {
  ...
  useEffect(() => {
    ...
    const interactiveSel = "a, button, [role='button'], input, select, textarea, .nav__link, .nav__cta, .story__visual, .eco-card, .future-row, .future-tick, .eco-stage__nav-btn, [data-hover]";
    const textSel = "h1, h2, h3, h4, h5, h6, p, li, blockquote, span:not([class*='dot']):not([class*='icon']):not([style]), .story__title, .story__body, .eco-stage__title, .eco-stage__body, .future-stage__title, .future-stage__desc, .hero__sub, .emotional__quote-body, .cta__sub";
    const onOver = (e) => {
      const t = e.target;
      if (!t.closest) return;
